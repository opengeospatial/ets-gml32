...
public class VerifyXMLUtils {
	...
	public void getFragment_shorthandPointer() throws SAXException, IOException {
		Document doc = docBuilder.parse(this.getClass().getResourceAsStream("/features/FZK-Haus-LoD2-KIT.xml"));
		...
		assertNull("Duplicate node should have no parent.", elem.getParentNode());
		assertEquals("Element has unexpected local name.", "Polygon", elem.getLocalName());
	}

	@Test
	public void getPropertyValueInSameDoc_validShorthandPointer()
			throws SAXException, IOException, XPathExpressionException {
		URL url = this.getClass().getResource("/features/FZK-Haus-LoD2-KIT.xml");
		...
		assertEquals("Element has unexpected local name.", "Polygon", value.getLocalName());
	}
