...
public class VerifyValidationUtils {
	...
	@Test
	public void extract2SchemaReferences() throws FileNotFoundException, XMLStreamException {
		File xmlFile = new File("src/test/resources/Gamma-any.xml");
		Set<URI> xsdRefs = ValidationUtils.extractSchemaReferences(new StreamSource(xmlFile), null);
		assertEquals("Unexpected number of schema locations.", 2, xsdRefs.size());
		URI uri = xsdRefs.iterator().next();
		assertTrue("Expected library.xsd or gamma.xsd in URI.",
				uri.toString().endsWith("library.xsd") || uri.toString().endsWith("gamma.xsd"));
	}

	@Test
	public void extract5SchemaReferences() throws FileNotFoundException, XMLStreamException {
		File xmlFile = new File("src/test/resources/basic-message.xml");
		Set<URI> xsdRefs = ValidationUtils.extractSchemaReferences(new StreamSource(xmlFile), null);
		assertEquals("Unexpected number of schema locations.", 5, xsdRefs.size());
		assertTrue("URI 'http://www.w3.org/1999/xlink.xsd' not found in set.",
