...
public class VerifyFeatureTypeFilter {
	...
	public void filterAnonFeatureType() throws SAXException, IOException {
		...
		Set<XSComplexTypeDefinition> complexTypes = XMLSchemaModelUtils.getReferencedComplexTypeDefinitions(model);
		FeatureTypeFilter iut = new FeatureTypeFilter();
		iut.filterSet(complexTypes);
		assertEquals("Unexpected number of feature types remain in set.", 1, complexTypes.size());
		assertTrue("Expected anonymous type definition.", complexTypes.iterator().next().getAnonymous());
	}
	...
	public void filterGlobalFeatureType() throws SAXException, IOException {
		...
		FeatureTypeFilter iut = new FeatureTypeFilter();
		iut.filterSet(complexTypes);
		assertEquals("Unexpected number of feature types remain in set.", 2, complexTypes.size());
	}
