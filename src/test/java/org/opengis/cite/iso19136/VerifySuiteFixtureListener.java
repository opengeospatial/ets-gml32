...
public class VerifySuiteFixtureListener {
	...
	public void processIUTParameter() throws URISyntaxException {
		...
		SuiteFixtureListener iut = new SuiteFixtureListener();
		iut.onStart(suite);
		verify(suite).setAttribute(ArgumentMatchers.eq(SuiteAttribute.SCHEMA_LOC_SET.getName()), uriSet.capture());
		assertFalse("Expected non-empty set of URIs.", uriSet.getValue().isEmpty());
	}
