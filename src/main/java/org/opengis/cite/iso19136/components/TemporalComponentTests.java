 ...
 * {@code gml:AbstractTimeObject}, which acts as the head of a substitution group for all
 * temporal primitives and complexes; the content model must be derived from
 * {@code gml:AbstractTimeObjectType}.
 *
 ...
 * </p>
 * <ul>
 * <li>ISO 19136:2007, cl. 14: GML schema – temporal information and dynamic features</li>
...
public class TemporalComponentTests extends SchemaModelFixture {
	...
	/**
	 * Determines if the application schema contains any temporal components (elements or
	 * type definitions). If not, all tests in this group are skipped.
	...
	public void hasTemporalComponents(ITestContext testContext) {
		...
		timeBaseType = this.model.getTypeDefinition(GML32.ABSTRACT_TIME_TYPE, GML32.NS_NAME);
		// types derived by extension from gml:AbstractTimeObjectType
		List<XSElementDeclaration> timeElements = XMLSchemaModelUtils.getGlobalElementsByType(this.model, timeBaseType);
		XSElementDeclaration abstractTime = this.model.getElementDeclaration(GML32.ABSTRACT_TIME, GML32.NS_NAME);
		...
		if (timeElements.isEmpty() && timeProps.isEmpty()) {
			throw new SkipException("No temporal (property) components found in schema.");
		}
	}

	/**
	 * [{@code Test}] All temporal types (elements) declared in an application schema must
	 * substitute for {@code gml:AbstractTimeObject}.
	 *
	...
	public void substitutesForAbstractTimeObject() {
		...
		Set<XSTypeDefinition> timeTypeDefs = XMLSchemaModelUtils.getDerivedTypeDefinitions(this.model, timeBaseType,
		...
		for (XSTypeDefinition typeDef : timeTypeDefs) {
			...
			for (XSElementDeclaration timeDecl : timeDecls) {
				Assert.assertTrue(timeElems.contains(timeDecl), ErrorMessage.format(ErrorMessageKeys.SUBSTITUTION_ERROR,
						new QName(timeDecl.getNamespace(), timeDecl.getName()), "gml:AbstractTimeObject"));
			}
		}
	}

	/**
	 * [{@code Test}] The value of a temporal property is an element substitutable for
	 * gml:AbstractTimeObject. Temporal properties may be defined explicitly (based on a
	 * pre-defined GML temporal property type) or implicitly by mimicking the standard GML
	 * property type content model.
	 *
	 * <p style="margin-bottom: 0.5em">
	 * <strong>Sources</strong>
	 * </p>
	...
	public void validateImplicitTemporalProperty() {
		XSElementDeclaration temporal = this.model.getElementDeclaration(GML32.ABSTRACT_TIME, GML32.NS_NAME);
		List<XSElementDeclaration> timeProps = XMLSchemaModelUtils.getImplicitProperties(model, temporal);
