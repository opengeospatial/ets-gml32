...
/**
 * Validates the content of a gml:Point element (or any element in its substitution
 * group), which implements the GM_Point class from ISO 19107. A point is defined by a
...
public class PointTests extends DataFixture {
	...
	/**
	 * A configuration method ({@code BeforeClass}) that looks for gml:Point elements in
	...
	public void findPoints() {
		...
		if (null != this.model) {
			XSElementDeclaration gmlPoint = this.model.getElementDeclaration(GML32.POINT, GML32.NS_NAME);
		...
		String xpath = generateXPathExpression(this.pointElems, namespaceBindings);
		try {
		...
		Assert.assertFalse(this.points.getLength() == 0, "gml:Point elements not found.");
	}

	/**
	 * [{@code Test}] Verifies that a gml:Point element has a valid CRS reference.
	 *
	 * <p style="margin-bottom: 0.5em">
	 * <strong>Sources</strong>
	 * </p>
	...
	/**
	 * [{@code Test}] Verifies the coordinates of a gml:Point. All of the following
	 ...
	 *
	 * <p>
	 * Note that the last constraint will detect obvious cases where the axis order is
	 * incorrect (e.g. a position expressed in EPSG 4326 as -122.22, 50.55).
	 * </p>
	 *
	 * <p style="margin-bottom: 0.5em">
	 * <strong>Sources</strong>
	...
	public void pointHasValidPosition() {
		for (int i = 0; i < this.points.getLength(); i++) {
			...
			try {
				...
				throw new AssertionError(ErrorMessage.format(ErrorMessageKeys.COORD_DIM_ERR, point.getLocalName(),
						point.getAttributeNS(GML32.NS_NAME, "id"), GmlUtils.findCRSReference(point)));
			}
			Assert.assertEquals(dpos.getDimension(),
					dpos.getCoordinateReferenceSystem().getCoordinateSystem().getDimension(),
					ErrorMessage.format(ErrorMessageKeys.COORD_DIM_ERR, point.getLocalName(),
							point.getAttributeNS(GML32.NS_NAME, "id"), GmlUtils.findCRSReference(point)));
			ImmutableEnvelope validArea = new ImmutableEnvelope(
					CRS.getDomainOfValidity(dpos.getCoordinateReferenceSystem()));
			Assert.assertTrue(validArea.contains(dpos),
					String.format("%s[@gml:id='%s'] is not within CRS area of use: %s.", point.getLocalName(),
							point.getAttributeNS(GML32.NS_NAME, "id"), validArea.toString()));
		}
	}

	/**
	 * Creates a DirectPosition object from a gml:Point element. It will have the same CRS
	 * as the given point; the reference system may be inherited from a containing
	 * geometry (aggregate) or feature.
	 * @param point A gml:Point element (or an element in its substitution group).
	 * @param ignoreThirdDimension If this flag is true then it will ignore third
	 * dimension else it will include all dimension.
	 * @throws IndexOutOfBoundsException If the length of the coordinate tuple exceeds the
	 * dimension of the associated CRS.
	...
	DirectPosition createDirectPosition(Element point, boolean ignoreThirdDimension) {
		...
		if (posList.getLength() != 1) {
			throw new IllegalArgumentException(
					"Expected point geometry containing exactly 1 gml:pos element; received " + point.getNodeName());
		}
		...
		try {
			...
			TestSuiteLogger.log(Level.WARNING, String.format("Unknown srsName found in %s[@gml:id='%s']: %s",
					point.getLocalName(), point.getAttributeNS(GML32.NS_NAME, "id"), srsName), fex);
		}
