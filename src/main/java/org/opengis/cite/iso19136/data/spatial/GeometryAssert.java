...
public class GeometryAssert {
	...
	/**
	 * Asserts that the given geometry element has a valid CRS reference. The value of the
	 * srsName attribute is an absolute URI that either identifies a "well-known" CRS
	 * definition or refers to a CRS definition. The reference may be inherited from a
	 * containing geometry (aggregate) or feature envelope.
	 * @param geom A DOM Element node representing a GML geometry element (or an element
	 * that can be substituted for one).
	 *
	...
	/**
	 * Asserts that the given geometry element is covered by the valid area of its
	 * associated CRS.
	 *
	 * <p>
	...
	/**
	 * Asserts that the number of direct positions in the posList element appearing within
	 * each segment of the given curve geometry satisfies the minimum length requirements.
	 *
	 * The number of entries in a posList element is equal to the product of the
	 * dimensionality of the CRS and the number of direct positions. An additional check
	 * ensures that the number of entries is consistent with the CRS even if the minimum
	 * length constraint is satisfied; that is, <code>size(posList) % dimCRS = 0</code>.
	 * @param gmlCurve An Element node representing a GML curve geometry (gml:Curve or
	 * gml:LineString)
	 */
	...
	/**
	 * Asserts that all segments of a curve are connected; that is, the end point of each
	 * segment (except the last) is identical to the start point of the next segment.
	 * @param gmlCurve An Element node representing a gml:Curve element.
	 */
	...
	/**
	 * Asserts that the components of a composite curve are connected; that is, the
	 * orientation of the curve members is such that each component (except the first)
	 * begins where the preceding one ends.
	 * @param gmlCurve An Element node representing a gml:CompositeCurve element.
	 */
	@SuppressWarnings("unchecked")
	 ...
	 * @param tolerancePPM The maximum tolerable difference between tuple elements,
	...
	/**
	 * Asserts that the boundary of the given surface is topologically correct. Each
	 * boundary component (curve) must be:
	 * <ol>
	 * <li><em>simple</em> (does not self-intersect at points other than boundary points),
	 * and</li>
	 ...
	 * @param surfaceElem A DOM Element node representing a surface geometry (substitutes
	 * for gml:AbstractSurface).
	 */
	...
	/**
	 * Asserts that the boundary of the given surface is correctly oriented with respect
	 * to the upward normal. In essence, the interior of the surface (patch) is always to
	 * the left of a boundary curve. The following constraints must be satisfied:
	 * <ol>
	 * <li>the exterior ring is oriented in a counter-clockwise (CCW) direction, and</li>
	...
	static void assertValidSurfaceOrientation(Element surfaceElem) {
		...
		for (int j = 0; j < interiorProps.getLength(); j++) {
			...
			Assert.assertFalse(CGAlgorithms.isCCW(interiorCoords), ErrorMessage
				.format(ErrorMessageKeys.INT_BOUNDARY_ORIENT, surfaceElem.getAttributeNS(GML32.NS_NAME, "id"), j + 1));
		}
	}
