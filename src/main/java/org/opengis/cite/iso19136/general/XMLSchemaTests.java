...
public class XMLSchemaTests {
	 ...
	 * {@link SuiteAttribute#SCHEMA_LOC_SET} should evaluate to a Set of URI objects
	 * specifying the locations of the relevant XML Schema grammars.
	...
	public void getSchemaURIsFromTestContext(ITestContext testContext) {
		...
		if ((null != uriSet) && Set.class.isAssignableFrom(uriSet.getClass())) {
			...
			throw new MissingResourceException("Unable to obtain XML Schema locations from ITestContext",
					SuiteAttribute.SCHEMA_LOC_SET.getType().getName(), SuiteAttribute.SCHEMA_LOC_SET.getName());
		}
	}

	/**
	 * [{@code @Test}] Verifies the validity of the GML application schema XML document
	 * with respect to the W3C XML Schema specification. It must satisfy all relevant
	 * constraints imposed by the XML Schema specification.
	 *
	 * <p>
	 * If the schema is successfully compiled with no errors it is added to the ISuite
	 * context as the value of the {@link SuiteAttribute#SCHEMA schema} attribute. The
	 * resulting (immutable, thread-safe) Schema object may then be used to construct a
	 * Validator.
	 * </p>
	 *
	 * <p>
	 * A fail verdict is produced if it takes longer than {@value #COMPILE_TIMEOUT} ms to
	 * compile the schema(s).
	 * </p>
	 *
	 * @see "ISO 19136:2007, cl. A.1.1.4 (Valid XML Schema)"
	 * @param testContext The test (group) context.
	 * @throws SAXException If a schema cannot be read.
	 * @throws IOException If a schema resource cannot be accessed for any reason.
	 */
	...
	public void compileXMLSchema(ITestContext testContext) throws SAXException, IOException {
		...
		// If this ETS is used by another one, the version parameter is most likely not
		// set
		...
		switch (gmlVersion) {
			case V321:
				entityCatalog = this.getClass().getResource(ETS_ROOT_PKG + "schema-catalog-gml-3.2.1.xml");
				break;
			case V322:
				entityCatalog = this.getClass().getResource(ETS_ROOT_PKG + "schema-catalog.xml");
				break;
			default:
				entityCatalog = this.getClass().getResource(ETS_ROOT_PKG + "schema-catalog.xml");
				break;
		...
		Schema schema = xsdCompiler.compileXmlSchema(xsdLocations.toArray(new URI[xsdLocations.size()]));
		Assert.assertNotNull(schema, ErrorMessage.format(ErrorMessageKeys.SCHEMA_ERROR, xsdLocations));
		ValidationErrorHandler errHandler = xsdCompiler.getErrorHandler();
		Assert.assertFalse(errHandler.errorsDetected(),
	...
	/**
	 * Sets the application schema(s) to check. This method is intended only to facilitate
	 * unit testing.
